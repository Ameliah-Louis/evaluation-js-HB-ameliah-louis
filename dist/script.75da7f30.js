...
$(document).ready(function () {
  ...
  var Livre = /*#__PURE__*/function () {
    function Livre(titre, auteur, image, resume) {
    ...
    _createClass(Livre, [{
      key: "genererElementHTML",
      value: function genererElementHTML() {
        var livreCard = $('<div>').addClass('livre-card'); // Création d'une div pour représenter le livre

        var titreElement = $('<h3>').text(this.titre); // Création d'un élément h3 pour le titre
        livreCard.append(titreElement); // Ajout du titre à la div

        var auteurElement = $('<p>').text("Auteur: ".concat(this.auteur)); // Création d'un élément paragraphe pour l'auteur
        livreCard.append(auteurElement); // Ajout de l'auteur à la div

        var panel = $('<div>').addClass('panel').hide(); // Création d'un panneau (div) pour afficher l'image et le résumé (masqué par défaut)
        livreCard.append(panel); // Ajout du panneau à la div

        var imageElement = $('<img>').attr('src', this.image); // Création d'un élément image
        panel.append(imageElement); // Ajout de l'image au panneau

        var resumeElement = $('<p>').text("R\xE9sum\xE9: ".concat(this.resume)); // Création d'un élément paragraphe pour le résumé
        panel.append(resumeElement); // Ajout du résumé au panneau

        var deleteButton = $('<span>').addClass('delete-button').text('Supprimer');
  ...
  var livreForm = $('#livre-form');
  var livreListe = $('#livre-liste');
  livreForm.on('submit', function (e) {
    ...
    var livre = new Livre(titre, auteur, image, resume);

    // Génération de l'élément HTML représentant le livre
    var livreCard = livre.genererElementHTML();
